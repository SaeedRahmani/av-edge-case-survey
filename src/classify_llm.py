...
Enable by setting ANTHROPIC_API_KEY and calling with --use-llm in discover.py
(or run this file directly on data/discovered.csv).
"""
...
_SUBS = {cls: list(subs.keys()) for cls, subs in TAXONOMY.items()}
...
SYSTEM = (
    ...
    "a paper belongs in the survey and, if so, its class and subsection. Be "
...
def _prompt(title, abstract):
    taxo = "\n".join(
        f"- {cls}: {', '.join(subs)}" for cls, subs in _SUBS.items()
    )
    return (
        f"Taxonomy (class: subsections):\n{taxo}\n\n"
        f"Paper title: {title}\nAbstract: {abstract[:1200]}\n\n"
        "Respond with ONLY a JSON object: "
        '{"relevant": bool, "class": one of '
        f"{_CLASS_LIST}, "
        '"subsection": string or null, "confidence": 0-1, "reason": short string}'
    )


def classify_llm(records, model="claude-opus-4-8"):
    ...
    for r in records:
        ...
        try:
            ...
            j = {"relevant": None, "class": None, "subsection": None,
